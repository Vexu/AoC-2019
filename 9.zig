const IntComputer = struct {
    ...
    fn init(allocator: *std.mem.Allocator, memsize: usize) !IntComputer {
        return IntComputer{
            .allocator = allocator,
            .mem = try allocator.alloc(i64, memsize),
        };
    }

    fn deinit(ic: *IntComputer) void {
        ic.allocator.free(ic.mem);
    }

    fn load(ic: *IntComputer, prog: []const i64) void {
        std.mem.copy(i64, ic.mem, prog);
        std.mem.set(i64, ic.mem[prog.len..], 0);
        ic.hlt = false;
    }

    fn getVal(ic: *IntComputer, argIndex: u8) *i64 {
        const n: i64 = switch (argIndex) {
            1 => 100,
            2 => 1000,
            3 => 10000,
            else => unreachable,
        };
        const arg = ic.mem[ic.ip + argIndex];
        const mode = @rem(@divTrunc(ic.mem[ic.ip], n), 10);
        return switch (mode) {
            0 => &ic.mem[@intCast(usize, arg)],
            1 => &ic.mem[ic.ip + argIndex],
            2 => &ic.mem[@intCast(usize, ic.base + arg)],
            else => unreachable,
        };
    }

    fn op1(ic: *IntComputer) void {
        var val1 = ic.getVal(1).*;
        var val2 = ic.getVal(2).*;
        var res = ic.getVal(3);
        res.* = val1 + val2;
        ic.ip += 4;
    }

    fn op2(ic: *IntComputer) void {
        var val1 = ic.getVal(1).*;
        var val2 = ic.getVal(2).*;
        var res = ic.getVal(3);
        res.* = val1 * val2;
    ...
    fn op3(ic: *IntComputer) void {
        var val1 = ic.getVal(1);
        val1.* = ic.in;
        ic.ip += 2;
    }

    fn op4(ic: *IntComputer) void {
        var val1 = ic.getVal(1).*;
        ic.result = val1;
        ic.ip += 2;
    }

    fn op5(ic: *IntComputer) void {
        var val1 = ic.getVal(1).*;
        var val2 = ic.getVal(2).*;
        if (val1 != 0)
            ic.ip = @intCast(usize, val2)
        else
            ic.ip += 3;
    }

    fn op6(ic: *IntComputer) void {
        var val1 = ic.getVal(1).*;
        var val2 = ic.getVal(2).*;
        if (val1 == 0)
            ic.ip = @intCast(usize, val2)
    ...
    fn op9(ic: *IntComputer) void {
        var val1 = ic.getVal(1).*;
        ic.base += val1;
        ic.ip += 2;
    }

    fn run(ic: *IntComputer) void {
        var asked = true;
        while (true) {
            switch (@rem(ic.mem[ic.ip], 100)) {
                1 => ic.op1(),
                2 => ic.op2(),
                3 => if (asked) {
                    ic.op3();
